...
def run_detection(logs: list) -> list:
    ...
    rules = {
        ...
        "Multiple Failed Attempts": r"multiple failed login attempts",
        "Command Injection": r"(;|\||\&\&|\|\|).*(rm|wget|curl|bash|sh|python|perl)",
        "Path Traversal": r"(\.\./|\.\.\\)",
        "File Upload": r"(upload|file_put_contents|fwrite|move_uploaded_file)",
        "Information Disclosure": r"(phpinfo|version|debug|error)",
        "Authentication Bypass": r"(admin|root|administrator).*(12345|password|admin|test)",
        "Data Exfiltration": r"(outbound|external|exfil|download).*(data|file|database)"
    }
        ...
        for rule_name, pattern in rules.items():
            if re.search(pattern, log, re.IGNORECASE):
                alerts.append({
                    ...
                    "rule": rule_name,
                    "log": log,
                    "severity": self._get_severity_for_rule(rule_name)
                })
...
# -------------------------------------------------------------------
# 9) Hybrid Detection - Combine both approaches
# -------------------------------------------------------------------
def run_hybrid_detection(events, iocs, logs=None, geoip_lookup=None):
    """
    Run both advanced event-based detection and basic log-based detection
    """
    all_detections = []
    
    # Run advanced detection on events
    if events and iocs:
        advanced_detections = run_all_detections(events, iocs, geoip_lookup)
        all_detections.extend(advanced_detections)
    
    # Run basic detection on logs
    if logs:
        basic_detections = run_detection(logs)
        
        # Convert basic detections to same format as advanced detections
        for detection in basic_detections:
            formatted_detection = {
                "title": f"{detection['rule']} Detected",
                "rule_id": f"LOG-{detection['rule'].upper().replace(' ', '_')}",
                "technique": self._get_technique_for_rule(detection['rule']),
                "severity": detection['severity'],
                "log_line": detection['line'],
                "log_content": detection['log'],
                "event_id": f"log-line-{detection['line']}",
...
def _get_technique_for_rule(rule_name):
    ...
    technique_map = {
        ...
        "XSS Attack": "T1059",     # Command and Scripting Interpreter
        ...
        "Multiple Failed Attempts": "T1110",  # Brute Force
        "Suspicious IP": "T1071",  # Application Layer Protocol
        "File Upload": "T1135",    # Network Share Discovery
    ...
    return technique_map.get(rule_name, "T1059")  # Default to Command and Scripting Interpreter